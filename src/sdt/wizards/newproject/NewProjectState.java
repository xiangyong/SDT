...
public class NewProjectState implements NewWizardState {
	...
	public Change[] computeChanges() {
		...
		context.put("projectBundleName", NameUtil.cap(this.name.replace('-', ' ')));

		String defaultPackage = "com.alipay." + this.name.replace('-', '.');
		context.put("projectSymbolicName", defaultPackage);
		context.put("defaultPackage", defaultPackage);
		...
		Change[] changes = ChangeEngine.run(context, "proj/" + this.type);
		...
		if (fromSystem != null) {
			...
			TextFileChange change = ChangeEngine.createNewFileChange(file, b.toString());
